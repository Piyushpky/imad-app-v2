 ...
 */
@Component
public class Postcard_1_1_SignatureSchemeStrategy extends PostcardSignatureSchemeStrategy implements SignatureSchemeStrategy {
	
	private static final WPPLogger LOG = WPPLoggerFactory.getLogger(Postcard_1_1_SignatureSchemeStrategy.class);
	...
	public byte[] generateSignature(CipherData cipherData, CipherKey cipherKey, PostcardSignatureScheme postcardSignatureScheme) throws PostcardNonRetriableException {
		...
		switch (postcardSignatureScheme) {
			...
			case raw_rsassa_pss:
				privateKey = ((RSACipherKey) cipherKey).getPrivateKey();
				return generateKeySignature(cipherData, privateKey, PostcardConstants.RAW_RSASSA_PSS_ALGORITHM_NAME);
			case hmac_sha256:
	...
	public void verifySignature(CipherData cipherData, CipherKey cipherKey, byte[] signatureTobeValidated, PostcardSignatureScheme postcardSignatureScheme) throws PostcardNonRetriableException {
		...
		switch (postcardSignatureScheme) {
			...
			case raw_rsassa_pss:
				publicKey = ((RSACipherKey) cipherKey).getPublicKey();
				verifyKeySignature(cipherData, publicKey, signatureTobeValidated, PostcardConstants.RAW_RSASSA_PSS_ALGORITHM_NAME);
				break;
			case hmac_sha256:
