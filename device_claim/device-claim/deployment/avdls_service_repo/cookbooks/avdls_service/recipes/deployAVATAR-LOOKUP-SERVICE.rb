...
execute "Deleting all wars ... " do
    command "rm -rf #{node['tomcat']['catalina_base']}/webapps/#{node['avdls_service']['war_prefix']}*"
    cwd "#{node['tomcat']['catalina_base']}/webapps/"
    action :run
end

#Copy war to tomcat location
remote_file "#{node['tomcat']['catalina_base']}/webapps/#{node['avdls_service']['artifact']['name']}.war" do
  source "#{artifact_url}"
  mode "0764"
...
directory node['avdls_service']['keystore']['cert_directory'] do
...
include_recipe 'avdls_service::applyEnvironment'
